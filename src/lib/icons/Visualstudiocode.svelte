<script lang="ts">
  ...
  let { size = ctx.size || '24', role = ctx.role || 'img', color = ctx.color || 'currentColor', ariaLabel = 'Visual Studio Code', class: classname, ...restProps }: Props = $props();
</script>
...
@prop export let ariaLabel = 'cib visual studio code';
-->
